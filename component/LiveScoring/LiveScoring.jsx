  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 md:p-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {/* Scoring Table */}
        {liveData && scoringData.length > 0 && (
          <div className="bg-gradient-to-br from-slate-800/90 via-slate-800/80 to-slate-900/90 rounded-2xl p-6 md:p-8 shadow-2xl border border-slate-700/50 mb-8 backdrop-blur-sm">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">
                Live Match Statistics
              </h2>
              <div className="flex items-center gap-3">
                <span className="px-5 py-2 bg-gradient-to-r from-red-600 to-red-700 text-white text-sm font-black rounded-full animate-pulse shadow-xl border-2 border-red-500/50">
                  🔴 LIVE
                </span>
                <span className="text-slate-400 text-sm font-medium bg-slate-900/50 px-3 py-1.5 rounded-lg border border-slate-700">
                  Match ID: {matchId}
                </span>
              </div>
            </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gradient-to-r from-slate-700 via-slate-600 to-slate-700">
                    <th className="px-5 py-4 text-left text-white font-black text-sm md:text-base border-b-2 border-slate-500 sticky left-0 bg-slate-700 z-10">
                      Rank
                    </th>
                    <th className="px-5 py-4 text-left text-white font-black text-sm md:text-base border-b-2 border-slate-500">
                      Player Name
                    </th>
                    <th className="px-5 py-4 text-center text-white font-black text-sm md:text-base border-b-2 border-slate-500">
                      Kills
                    </th>
                    <th className="px-5 py-4 text-center text-white font-black text-sm md:text-base border-b-2 border-slate-500">
                      Assists
                ...
                <tbody>
                  {scoringData.map((player, index) => (
                    ...
                    >
                      <td className="px-5 py-4 text-white font-bold sticky left-0 bg-inherit z-10">
                        <span className={`inline-flex items-center justify-center w-10 h-10 rounded-full shadow-lg ${
                          index === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 text-black' :
                          index === 1 ? 'bg-gradient-to-br from-slate-400 to-slate-600 text-black' :
                      ...
                      <td className="px-5 py-4 text-center text-red-400 font-bold">
                        {getValue(player, 'kills', 'kill', 'k', 'killsCount')}
                      </td>
                      <td className="px-5 py-4 text-center text-blue-400 font-bold">
                        {getValue(player, 'assists', 'assist', 'a', 'assistsCount')}
                      </td>
                      <td className="px-5 py-4 text-center text-yellow-400 font-bold">
                        {getValue(player, 'damage', 'dmg', 'damageDealt')}
                      </td>
                      <td className="px-5 py-4 text-center text-green-400 font-bold">
                        {getValue(player, 'survival', 'survivalTime', 'time', 'survived')}
                      </td>
                      <td className="px-5 py-4 text-center text-purple-400 font-black text-lg">
                        {getValue(player, 'score', 'totalScore', 'points', 'totalPoints')}
                      </td>
                      <td className="px-5 py-4 text-center text-cyan-400 font-semibold">
                        {getValue(player, 'team', 'teamId', 'teamName', 'teamNumber')}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {/* Raw Data Display */}
        {liveData && scoringData.length === 0 && teams.length === 0 && (
          <div className="bg-gradient-to-br from-slate-800/90 to-slate-900/90 rounded-2xl p-6 shadow-xl border border-slate-700/50">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-black text-white">Raw Match Data</h2>
              <span className="px-4 py-1.5 bg-red-600 text-white text-sm font-black rounded-full animate-pulse">
                LIVE
              </span>
            </div>
            <div className="bg-slate-900/80 rounded-xl p-4 overflow-x-auto border border-slate-700">
        ...
        {!liveData && !loading && !error && (
          <div className="bg-gradient-to-br from-slate-800/90 to-slate-900/90 rounded-2xl p-12 text-center border border-slate-700/50 shadow-xl">
            <p className="text-slate-400 text-xl font-semibold">No data available. Click "Fetch Live Score" to load data.</p>
          </div>
        )}
      </div>
    </div>
